...
parser.add_argument('--paramsindex', type=int,nargs='+', required=False, help='paramsindex', default=[0,2,3])
...
elif pcinit == 'rand_zach':
        params = random_zach_pc_weights(npc, nact, seed, sigma=sigma, alpha=alpha, envsize=envsize)
...
def run_trial(params, env, trial):
    ...
    for t in range(tmax):
        ...
        params, td = learn_sigma(params, reward, newstate, state, onehotg,aprob, gamma, etas,balpha, noise, paramsindex)
...
for episode in range(train_episodes):
    ...
    print(f'Trial {episode+1}, G {np.sum(discount_rewards):.3f}, t {latency}, L {tds:.3f}, a {np.linalg.norm(params[2],ord=1):.3f}')


#%%
env.plot_trajectory()

f,score, drift = plot_analysis(logparams, latencys, allcoords, stable_perf=0, exptname=exptname, rsz=goalsize)
print(score, drift)
